...
const SAVE_PATH := "user://bisca.cfg"
const GAMES_TO_WIN := 4
const VARIANTS := [3, 7, 9]  # cards per player
...
const KING := 7
const ACE := 9
const TITLE_CARDS := [28, 8, 38, 18]  # the four sevens, the biscas
...
var deal_i := 0
var deal_t := 0.0
var draws := 0  # cards drawn from the stock after the last trick
...
var sound_on := true
var fast := false
var variant := 0  # index in VARIANTS; a change applies from the next hand
...
## True when card `a`, played after `b`, beats it.
...
## Cards `seat` may play: any card while the stock lasts or when leading; after that, the led suit
## if it has any.
func _legal(seat: int) -> Array:
	var hand: Array = hands[seat]
	if trick.is_empty() or not deck.is_empty():
...
## Whether `seat` may swap the 2 of trump for the face-up trump now.
func _can_swap(seat: int) -> bool:
	return phase == "play" and turn == seat and not deck.is_empty() and trump * 10 in hands[seat]
...
## Turns the next card face up and slides it under the stock: its suit is trump.
func _reveal() -> void:
...
## Swaps the 2 of trump in `seat`'s hand for the face-up trump under the stock.
func _swap(seat: int) -> void:
	var two := trump * 10
	hands[seat].erase(two)
	hands[seat].append(trump_card)
	deck[0] = two
	trump_card = two
	if seat == 0:
		_sort_hand()
...
func _unseen(seat: int) -> Array:
	var out := []
	for c in 40:
		if not played.has(c) and not c in hands[seat] and not (not deck.is_empty() and c == deck[0]):
			out.append(c)
	return out
...
## Whether any of `cards` can beat `card`, when played after it.
func _beaten_by(cards: Array, card: int) -> bool:
	var suit := card / 10
	var must_follow := deck.is_empty() and cards.any(func(u: int) -> bool: return u / 10 == suit)
	for u: int in cards:
		if u / 10 == suit and u % 10 > card % 10:
			return true
		if u / 10 == trump and suit != trump and not must_follow:
			return true
	return false


## How good `c` is as the first card of a trick: lead cheap cards and keep trumps and points.
func _lead_score(c: int, unseen: Array) -> float:
	var pts := _points(c)
	var rank := c % 10
	var score := -pts * 1.2 - rank * 0.15
	if c / 10 == trump:
		score -= 5.0 + rank * 0.5 + pts
	# With the stock gone the other hand is known: cash the cards it cannot beat.
	if deck.is_empty() and not _beaten_by(unseen, c):
		score += 14.0 + pts * 2.5
	return score


## How good `c` is as the answer to the card led: the points it wins or gives away, minus the
## cost of spending a trump.
func _follow_score(c: int) -> float:
	var lead: int = trick[0].card
	var pts := _points(c) + _points(lead)
	var rank := c % 10
	var spend := 0.0
	if c / 10 == trump:
		spend = 3.0 + rank * 0.6 + _points(c)
		if lead / 10 == trump:
			spend *= 0.5
...
## the computer searches every way the hand can go.
func _solve(seat: int, legal: Array) -> int:
	var mine: Array = hands[seat].duplicate()
	var theirs := _unseen(seat)
	var lead: int = -1 if trick.is_empty() else trick[0].card
	var best: int = legal[0]
	var best_value := -INF
	for c: int in legal:
		var value := _after_move(mine, theirs, true, lead, c, -INF, INF) + rng.randf() * 0.01
		if value > best_value:
			best_value = value
			best = c
	return best
...
## Points the computer ("me") still wins from here with best play on both sides. `my_move` says
## who plays next and `lead` is the card already on the table, or -1.
func _minimax(me: Array, them: Array, my_move: bool, lead: int, alpha: float, beta: float) -> float:
	var hand: Array = me if my_move else them
	if hand.is_empty():
		return 0.0
	var moves := hand
	if lead >= 0:
		var follow := hand.filter(func(c: int) -> bool: return c / 10 == lead / 10)
		if not follow.is_empty():
			moves = follow
	var best := -INF if my_move else INF
	for c: int in moves:
		var value := _after_move(me, them, my_move, lead, c, alpha, beta)
		if my_move:
...
func _after_move(me: Array, them: Array, my_move: bool, lead: int, c: int, alpha: float, beta: float) -> float:
	var me2 := me.duplicate()
	var them2 := them.duplicate()
	if my_move:
		me2.erase(c)
	else:
		them2.erase(c)
	if lead < 0:
		return _minimax(me2, them2, not my_move, c, alpha, beta)
	var i_win := _beats(c, lead) == my_move
	var gained := float(_points(c) + _points(lead)) if i_win else 0.0
	return gained + _minimax(me2, them2, i_win, -1, alpha - gained, beta - gained)
...
func _menu_rows() -> Dictionary:
	...
	return {
		...
		"variant": Rect2(x, y + 184, w, 48),
		"rules": Rect2(x, y + 240, w, 48),
		"new_match": Rect2(x, y + 530, w, 48),
	}
...
func _menu_click() -> void:
	...
	if rows.close.has_point(mouse) or not MENU_RECT.has_point(mouse):
		...
		sfx.play("click")
		_save()
	elif rows.variant.has_point(mouse):
...
func _save() -> void:
	...
	config.set_value("settings", "variant", variant)
...
func _load() -> void:
	...
	variant = clampi(int(config.get_value("settings", "variant", 0)), 0, VARIANTS.size() - 1)
...
func _draw_cards() -> void:
	...
	var selected := _selected()
	var swappable := _my_turn() and _can_swap(0)
	for c: int in order:
		var v: Dictionary = vis[c]
		...
		if c == winner_card:
			...
			glow = 0.55
		elif c == trump_card and not deck.is_empty():
			if swappable:
				glow = 0.8 + 0.2 * sin(clock * 5.0)
			elif phase == "reveal":
				glow = 0.7 + 0.3 * sin(clock * 3.0)
...
func _draw_message() -> void:
	...
	if message == "your_turn":
		...
		col = Color("ffb4a8")
	elif message in ["stock_empty", "swapped_you", "swapped_other"]:
		col = GOLD_LIGHT
...
## "Bisca" in gold, centered on `center`.
func _draw_logo(center: Vector2, s: float) -> void:
	_text_fx("Bisca", center, int(128 * s), GOLD, int(7 * s))
...
func _draw_menu() -> void:
	...
	draw_line(c + Vector2(9, -9), c + Vector2(-9, 9), GOLD_LIGHT if hot_close else INK, 2.5, true)
	for key in ["sound", "speed", "variant", "rules"]:
	...
	_text_right("‹  " + tr("variant_%d" % VARIANTS[variant]) + "  ›", Vector2(rows.variant.end.x - 16, rows.variant.get_center().y), 17, GOLD_LIGHT, bold)
	_text_right("›", Vector2(rows.rules.end.x - 16, rows.rules.get_center().y), 22, GOLD_LIGHT, bold)
	if phase != "title" and VARIANTS[variant] != hand_size and not (phase == "round_end" and match_over):
		_text(tr("variant_next"), Vector2(MENU_RECT.get_center().x, rows.variant.end.y + 1), 11, MUTED, font)

	_text_left(tr("statistics"), Vector2(x, y + 322), 20, GOLD, serif)
	...
	_text(tr("keys_help"), Vector2(MENU_RECT.get_center().x, y + 610), 12, MUTED, font)
...
func _draw_icon() -> void:
	...
	var pivot := Vector2(256, 350)
	var cards := [28, 9, 18]  # 7 of spades, ace of hearts, 7 of diamonds
	for i in 3:
		var a := (i - 1) * 0.34
		_draw_card(cards[i], pivot + Vector2(0, -150).rotated(a), a, 1.5, 1.0)
	_draw_logo(Vector2(256, 420), 1.02)
...
func _draw_art() -> void:
	match art_kind:
		"box":
			_draw_fan(TITLE_CARDS, Vector2(540, 740), 330, 2.3, 0.3)
			_draw_twinkles(Vector2(540, 800), Vector2(450, 110), 16)
			_draw_logo(Vector2(540, 800), 1.9)
			_text(tr("subtitle"), Vector2(540, 962), 34, MUTED, font)
		"poster":
			_draw_fan(TITLE_CARDS, Vector2(360, 590), 270, 1.8, 0.3)
			_draw_twinkles(Vector2(360, 690), Vector2(300, 90), 12)
			_draw_logo(Vector2(360, 690), 1.35)
			_text(tr("subtitle"), Vector2(360, 800), 23, MUTED, font)
		...
		"hero":
			...
			_draw_logo(Vector2(1320, 520), 1.9)
			_text(tr("subtitle"), Vector2(1320, 680), 36, MUTED, font)
